...
# move the ubuntu-base-ov-gtsam-gemm to the firechip directory
if [ -f "$ROOT_DIR/temp/ubuntu-base-ov-gtsam-gemm.img" ]; then
    echo "Moving ubuntu-base-ov-gtsam-gemm.img to firechip directory..."
    mv $ROOT_DIR/temp/ubuntu-base-ov-gtsam-gemm.img $CY_DIR/software/firemarshal/images/firechip/ubuntu-base/ || { echo "Failed to move ubuntu-base-ov-gtsam-gemm.img"; exit 1; }
else
    echo "ubuntu-base-ov-gtsam-gemm.img not found in temp directory."
fi
...
cp $ROOT_DIR/firesim-settings/ov-gtsam-gemm/ubuntu-base.json deploy/workloads/ubuntu-base.json || { echo "Failed to copy ubuntu-base.json"; exit 1; }

# copy the config_runtime.yaml to the firesim directory
cp $ROOT_DIR/firesim-settings/ov-gtsam-gemm/config_runtime.yaml $CY_DIR/sims/firesim/deploy/config_runtime.yaml || { echo "Failed to copy config_runtime.yaml"; exit 1; }
