...
cp $ROOT_DIR/firesim-settings/vio-gemmini/ubuntu-base.json deploy/workloads/ubuntu-base.json || { echo "Failed to copy ubuntu-base.json"; exit 1; }

# copy the config_runtime.yaml to the firesim directory
cp $ROOT_DIR/firesim-settings/vio-gemmini/config_runtime.yaml $CY_DIR/sims/firesim/deploy/config_runtime.yaml || { echo "Failed to copy config_runtime.yaml"; exit 1; }
